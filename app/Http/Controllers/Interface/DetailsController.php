...
class DetailsController extends Controller
{
    ...
    {
        $productAndScheduleExist = DB::table('products') //lấy id từ 2 bảng product và schedule gán cho biến $productAndScheduleExist
            ->where('id', $product_id)
            ->exists()
            && DB::table('schedule')
        ...
        if (!$productAndScheduleExist) {    // kiểm tra xem nếu $productAndScheduleExist không tồn tại
            return redirect()->route("gd.index_tour",0);
        }
